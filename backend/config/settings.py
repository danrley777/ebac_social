...
ALLOWED_HOSTS = os.getenv("DJANGO_ALLOWED_HOSTS", "localhost 127.0.0.1 [::1]").replace(",", " ").split()
...
if DATABASE_URL:
    ...
    if db.scheme not in {"postgres", "postgresql"} or not db.hostname or not db.path.strip("/"):
        raise ImproperlyConfigured("DATABASE_URL inválida")
    DATABASES = {"default": {"ENGINE": "django.db.backends.postgresql", "NAME": unquote(db.path.lstrip("/")),
        ...
        "PORT": db.port or 5432, "OPTIONS": dict(parse_qsl(db.query)), "CONN_MAX_AGE": 60, "CONN_HEALTH_CHECKS": True}}
else:
...
CORS_ALLOWED_ORIGINS = os.getenv("CORS_ALLOWED_ORIGINS", "http://localhost:5173").replace(",", " ").split()
REST_FRAMEWORK = {
